...
> **「友人」「視聴者」は何のためにあるか** — 本システムは商用サービスではなく、番組の感想やソース選定のフィードバックを得るための少人数運用です。「友人(subscriber)」はポッドキャストの購読トークンを持つ人、「視聴者(viewer)」は**ソース一覧の閲覧だけができる**アカウントで、記事本文にも要約にも到達できません(D-27)。配信先を絞っているのは規模の都合ではなく、次節のとおり意図的な設計判断です。

### コンテンツの取り扱いと配信範囲

本システムは公開されている記事・動画・音声を収集し、**本文を保存したうえで要約・台本化し、音声にして配信**します。原典の権利はそれぞれの提供元にあり、この処理は複製・翻案・公衆送信に該当し得ます。日本の著作権法に包括的なフェアユース規定はなく、権利制限規定は個別に定められた場面でしか働きません。軽微利用を認める著作権法 47 条の 5 も、所在検索・情報解析の結果提供に**付随する**必要な限度の利用を対象とするもので、要約・台本・音声そのものを届ける本システムはその射程外です(同条をめぐって引かれる「記事本文の 5〜10% 程度」〔日本新聞協会〕も検索結果表示についての目安であり、要約一般の安全域ではありません)。つまり配信の根拠を権利制限規定に求めることはできず、**提供元の許諾と利用規約の確認が前提**になります。そのうえで、機能ではなく**スコープの側で制約を置いています**。

- **配信先は特定少数に限定** — トークンを発行した本人と、フィードバックを依頼する少数の友人のみ。一般公開・商用配信・広告掲載は行いません。著作権法上の「公衆」は不特定または特定多数を指すため、ここを広げると前提が崩れます
- **原典への導線を必ず併記** — 要約・台本は原典の理解を助けるためのもので、原典の代替を意図しません
- **書籍・私的データはローカル LLM のみ** — 書籍 PDF は本人の蔵書に限り、取り込みも要約も Mac のローカル LLM で完結させて外部 API に送らず、生成物は私的フィード限定で友人には配信しません(C-5 / C-12)
- **クロールは名乗って行う** — 全リクエストに `catchup-feed/1.0 (personal RSS reader)` を User-Agent として送出します(robots.txt の遵守は未実装で、次サイクルの課題)
- **提供元の規約が著作権法とは別に効く** — RSS の利用規定で再配信・加工を禁じている提供元や、ダウンロードを規約で禁じているプラットフォームがあります。ソース追加時の確認事項です

利用者向けの同趣旨の記載はアプリ内の `/terms`(3. コンテンツの権利)にもあります。本節は法的助言ではなく、個人開発における設計判断の記録です。
